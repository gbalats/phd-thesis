...
subtree or a portion of a list may be shared, and so on.
...
Moreover, at each memory state the analysis has computed, it tends to
create multiple memory states, by trying to collapse \(1\over{2}\)
values of properties to either 0 or 1 (thus, to eliminate
uncertainty)---via the so called \emph{focus operation}. An abstract
object is defined only by the values it has for the basic properties
(called \emph{abstraction properties}) defined by the particular
analysis. Therefore, the upper bound for the number of abstract
objects in memory is exponential with respect to the number of
abstraction predicates defined. Inconsistent states are then discarded
at the \emph{coerce operation}.
