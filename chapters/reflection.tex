...
analysis for Java and discuss the fundamental differences from
analysis of C/C++.
...
relevant to a points-to analysis. We note some basic differences from
the C/C++ setting:
\begin{itemize}[--]
...
Figure~\ref{reflection/fig/javapointsto} presents a typical points-to
analysis, expressed in inference rules as those of
Chapter~\ref{chapter:structsens}. The basic relations it computes
are:
\begin{description}
...
a single kind of abstract object that represents heap allocations
...
checking if the type of the allocation is a subtype of the variable's
type). Since Java is strongly-typed, this is the only place where we
can benefit from such a type check; any other points-to edges
...
setting), regarding their arguments and returned values.
...
a relation \pred{CallGraphEdge}{i: I, m: M}. Intuitively, the
