...
\code{Class.forName} library call, the static analysis
%has very few
%available courses of action: It 
needs to either conservatively
over-approximate (e.g., assume that \emph{any} class can be accessed),
%%SPACE
%possibly limiting the set later, after the returned object is used),
or to perform a string analysis that will allow it to infer the
contents of the \code{forName} string argument. Both options can be
...
%paper serve as a step in that direction: beyond soundy, yet not fully
%sound.
...
%
%\begin{quote}
%\emph{Hsqldb and jython could not be analyzed with reflection analysis
%enabled [...]
%%---hsqldb cannot even be analyzed context-insensitively and
%%jython cannot even be analyzed with the 1obj analysis. This is 
%due to vast imprecision introduced when reflection methods are not
%filtered in any way by constant strings (for classes, fields, or
%methods) and the analysis infers a large number of reflection objects
%to flow to several variables.}
%% [...]  For these two applications, our analysis has
%% reflection reasoning disabled. 
%%Since hsqldb in the DaCapo benchmark
%%code has its main functionality called via reflection, we had to
%%configure its entry point manually.}
%\end{quote}

%\noindent
In this paper, we describe an approach to analyzing reflection in the
...
%%\code{Class.getMethod}) that attempts to retrieve a method from a class
...
%%operation (e.g., the return value of a \code{Field.get} or a
%%\code{Method.invoke} call) then we immediately get information on the
...
%%just from casts. For instance, a \code{Class.getMethod} or
%%\code{Class.getField} call with a constant string parameter gives
...
%%%SPACE
%The rest of the paper introduces important background on static
%reflection analysis (Section~\ref{sec:model}), presents our new
%techniques (Section~\ref{soundiness}), evaluates them
%(Section~\ref{sec:experiments}), and compares to related work
...
%Domains: 
%I  -- instructions (invocation sites)
%N  -- natural numbers
%V  -- variables
%string -- strings
%T  -- types
%H  -- heap object abstractions, i.e., allocation sites
%S  -- method signatures
%M  -- methods
...
\code{Class.forName}), creating a new object given a class object
(library method \code{Class.newInstance}), retrieving a reflective
method object given a class object and a signature (library method
\code{Class.getMethod}), and reflectively calling a virtual method on
an object (library method \code{Method.invoke}). This treatment
ignores several other APIs, which are handled similarly. These
include, for instance, fields, constructors, other kinds of method
invocations (static, special), reflective access to arrays, other ways
to get class objects, and more.
...
%\begin{tabular*}{0.9\columnwidth}{@{\extracolsep{\fill}}llll}
%$+ \transform + = +$ & $- \transform + = -$ & $* \transform + = *$ & $o \transform + = o$\\
%$+ \transform - = -$ & $- \transform - = +$ & $* \transform - = *$ & $o \transform - = o$\\
%$+ \transform * = *$ & $- \transform * = *$ & $* \transform * = *$ & $o \transform * = *$\\
%$+ \transform o = o$ & $- \transform o = o$ & $* \transform o = *$ & $o \transform o = o$
%\end{tabular*}
%\end{minipage}
%}
...
supplies more rules for computing a relation \pred{VarPointsTo}{v: V,
  h: H} and a relation \pred{CallGraphEdge}{i: I, m: M}. Intuitively,
...
\tab \pred{Call}{i, \code{"Class.newInstance"}}, \pred{ActualArg}{i, 0, v}, 
     \pred{VarPointsTo}{v, h_c},\\
\tab \pred{ReifiedClass}{t, h_c}, \pred{AssignRetValue}{i, r}, 
     \pred{ReifiedHeapAllocation}{i, t, h}.\\
\end{rules}
...
\tab \pred{Call}{i, \code{"Class.getMethod"}}, \pred{ActualArg}{i, 0, b}, \pred{ActualArg}{i, 1, p}, \\
\tab \pred{AssignRetValue}{i, r}, \pred{VarPointsTo}{b, h_c}, \pred{ReifiedClass}{t, h_c},\\
\tab \pred{VarPointsTo}{p, c}, \pred{ConstantForMethod}{c, s}, \\ % param points to string constant
\tab \pred{Lookup}{t, s, \_}, \pred{ReifiedMethod}{s, h_m}.\\ % // signature agrees with type
\end{rules}
...
\tab \pred{Call}{i, \code{"Method.invoke"}}, \pred{ActualArg}{i, 0, b}, \pred{ActualArg}{i, 1, p}, \\
...
%%SPACE
%The above five rules are a small part of a realistic implementation of
%reflection handling, but they offer a faithful model of the core of
%the analysis---other additions 
%handle more reflective calls and more
%language types (e.g., arrays) but 
%represent engineering, rather than
%conceptual handling. 
%%SPACE
%Having declarative rules allows easy inspection of changes. 
%Note how much of the logic relies on inter-procedural
%properties (i.e., \predname{VarPointsTo} information), and at the same
%time produces inter-procedural properties (\predname{VarPointsTo} and
%\predname{CallGraphEdge}).
...
to a \code{forName} call. The parameter of \code{forName} could be any
...
calls---we already encountered the \code{getMethod} call in
Section~\ref{sec:model}. 

%%%SPACE
%% \paragraph{Example.} 
%% The (simplified) code excerpt shown in Figure~\ref{figure:substrings},
...
%%    Method meth = 
%%        el.getClass().getMethod(setterString, argTypes);
%%    meth.invoke(el, args);
%% }
...
%% %narrow down the setter methods to be called to just one, but this would
%% %require sophisticated reasoning about the computation performed inside
%% %\code{getSetterMethodName()}. Such reasoning is outside the scope of this
%% %paper and fairly foreign to scalable over-approximate techniques, such as
%% %pointer analysis. Furthermore, the exact value of \code{m\_name} could be missing
%% %or merged with many other (irrelevant) string constants. 
...
%These settings reflect a balance between expected usage
...
\begin{rules}
\pred{VarPointsTo}{r, h} \rulearrow \\
\tab \pred{Call}{i_a, \code{"append"}}, \pred{ActualArg}{i_a, 0, v_f}, \pred{ActualArg}{i_a, 1, v}, \\
\tab \pred{StringFactoryVar}{v_f}, \pred{Call}{i_t, \code{"toString"}}, 
     \pred{AssignRetValue}{i_t, r}, \\
...
\footnote{E.g., 
%For instance, this is enabled with the flag
  \texttt{SMUSH\_STRINGS} in \textsc{Wala} 
  \cite{www:wala-reflection} and \texttt{MERGE\_STRING\_BUFFERS} in
   \textsc{Doop}.}
% Both flags are on by default for
%  precise (i.e., costly) analyses.}
...
%in the program.  We call the approach 
...
%% SPACE
%A static points-to analysis is primarily a \emph{may} analysis: it
...
under-approximated call can be a \code{Class.forName},
\code{Class.get[Declared]Method}, \code{Class.get[Declared]Field},
etc. call, which returns a dynamic representation of a class, method,
or field, given a string name.

%\begin{itemize}
%\item A \code{Class.forName} call, as seen earlier: returns a dynamic representation of a
%  class, given a string.
%\item A \code{Class.get[Declared]Method} call, as seen earlier: returns a dynamic
%  representation of a method, given a class and a string.
%\item A \code{Class.get[Declared]Field} call: returns a dynamic representation of a
%  field, given a class and a string.
%\item A \code{Class.get[Declared]Methods} call: returns all (or all public) methods
%  of a class.
%\item A \code{Class.get[Declared]Fields} call: returns all (or all public) fields of
%  a class.
%%%SPACE
...
\begin{itemize}
\item At the site of a \code{forName} call, create a marker
  object (of type \code{java.lang.Class}), to stand for all unknown
...
\item At the site of a \code{newInstance} invocation, if the receiver is
  our special object, the result of \code{newInstance} is also a special
  object (of type \code{java.lang.Object} this time) that remembers its
  \code{forName} origins.
\item This second special object also flows freely through the points-to analysis,
  taking full advantage of inter-procedural reasoning facilities.
\item If the second special object (of type \code{java.lang.Object})
  reaches the site of a cast, then the original \code{forName}
  invocation is retrieved and augmented to return the cast type or its
  subtypes as class objects.
\end{itemize}
...
\tab \pred{Call}{i, \code{"Class.forName"}}, \pred{AssignRetValue}{i, v}, \pred{ReifiedForName}{i, h}.\\
\end{rules}
...
\tab \pred{Call}{i_n, \code{"Class.newInstance"}}, \pred{ActualArg}{i_n, 0, v}, \pred{VarPointsTo}{v, h}, \\
\tab \pred{AssignRetValue}{i_n, r}, \pred{ReifiedForName}{i, h}, \pred{ReifiedNewInstance}{i, h_n}.
\end{rules}
...
\code{newInstance} will be another special object (of appropriate type---determined
by the contents of \code{ReifiedNewInstance})
...
\begin{rules}
\pred{ClassObject}{i, t'} \rulearrow \\
\tab \pred{Cast}{\_, v, t}, \pred{Subtype}{t', t}, \pred{VarPointsTo}{v, h_n}, \pred{ReifiedNewInstance}{i, h_n}.\\
\end{rules}
...
idea to the 4 API calls \code{Class.get[Declared]Method} and
\code{Class.get[Declared]Field}.
...
\item Our analysis generalizes the pattern significantly. In our
  ...
  % analysis, as we will see in Section~\ref{sec:throttling}.
  ...
  %   few mentioned in Section~\ref{sec:back-propagation}) and only
  %   resolves them when ...
...
%% topic in Section~\ref{sec:throttling}, but first we consider
...
\code{Class.newInstance}, although similar logic applies to reflection
calls such as \code{Constructor.newInstance}, as well as
\code{Method.invoke} and \code{Field.get}.
...
\tab \pred{ReifiedMarkerNewInstance}{i, h_m}, \pred{ReifiedInventedObject}{i, t, h}.
\end{rules}
...
%Although we compare to related work in Section~\ref{sec:related}
...
%relation, defined in Section~\ref{sec:model}. The rule's logic states that
...
%of Section~\ref{sec:model}) or if it is resolved to some type that cannot
...
%As discussed in Section~\ref{sec:back-propagation}, the
...
``\code{(Event)}'' is much more informative when \code{Event} is a class
with only a few subclasses, rather than when \code{Event} is an
...
object could have been returned by a \code{Class.forName}, the
back-propagation takes place only when the string name matches methods
of at most $d$ different types. This threshold approach minimizes the
potential for noise back-propagating and polluting all subsequent
program paths that depend on the original reflection call.
...
%% The evaluation of our techniques aims to answer three 
%% research questions:
...
%%SPACE
%, by instrumenting its execution.
...
%at determining what makes up the application
...
%SPACE
%%  We consider only call-graph edges originating from
%% application code, since library classes contain a fair amount of
%% non-analyzable native methods.\footnote{Call-graph edges from the
%%   library are still fully statically analyzed, thus our experiments
%%   demonstrate scalability relative to large libraries. We just do not
%%   report library-originating edges (though they are computed within
%%   the time reported) since these only cloud the picture, due to native
...
%% %
%% We also filter out some missing edges:
%% %%% SPACE 
%% %(i.e., consider them
%% %implicitly covered), which involve the following methods:
%% \begin{bullets}
%% %[$\cdot$]
%% \item \emph{Class Initializers.} 
%%   \textsc{Doop} only models 
...
%%   out edges to class initializer methods, if static analysis indicates
...
%Two research questions can now be answered:
...
%  Section~\ref{sec:throttling} manage to preserve scalability.
...
%  Section~\ref{sec:throttling}) they hardly show a difference over
...
%%SPACE
%it is not applicable in many static analysis settings. M
...
arguments
%%SPACE
% (in order to avoid writing verbose, formulaic code), 
...
% the possible values of arbitrary string expressions.

%%SPACE
%Ali and Lhot{\'a}k~\cite{ecoop/AliL12,ecoop/AliL13}
%offer comparisons of dynamic and static call-graph edge metrics.  They
...
%\code{Object.getClass}) offered by \textsc{Elf} or
%our current work. 
...
%(\emph{avrora, luindex, and lusearch}), 
...
%in Section~\ref{sec:back-propagation}. Furthermore, the Li et al.
