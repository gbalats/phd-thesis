...
% for practical purposes. Algorithm~\ref{alg:multiple:simple} performs
...
% Algorithm~\ref{alg:single:kdirect}. Such a sort makes two nodes
...
\item A similar kind of constraint at the Java language level is also
  ...
  Section~\ref{multiple}) can be satisfied by making \code{X} a
...
Section~\ref{discussion}, the input and output language assumptions
crucially determine the essence of an incomplete program analysis
problem.
